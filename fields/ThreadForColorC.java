...
public class ThreadForColorC extends Thread {
    ...
    public ThreadForColorC(int _ZOOM, double _gradient, Graphics2D _screen, int _precision, double _largestG, List<ArrayList<Double>> _list) {
        ZOOM = _ZOOM;
        gradient = _gradient;
        screen = _screen;
    ...
    public void run () {
        int x;
        int y;
        for (int i = 2 * list.size()/4; i < 3*list.size() / 4; i ++) {
